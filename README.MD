...
The code may priorize make all necessary calculations in *Python* and <a href="https://code.mpimet.mpg.de/projects/cdo"> CDO </a>.
...
## To run the code
```shell
	./heatwave.sh netcdf/tmax.nc 1990 95 15 [netcdf/pr.nc] [netcdf/tmin.nc] [netcdf/percenttmax.nc] [netcdf/percenttmin.nc]
```
...
If you don't have the file, pass some else. If is'not necessary, will not be loaded
...
- If maximum temperature named like mn2t, change to tmax
- If minimum temperature named like tas, change to tmin
- If time variable named like valid_time, change to time
...
2. <b> historical_(min|max).nc </b> is the file with old raw data that calculate heatwave index
3. <b> percent(min|max).nc </b> is the file with climatological model to each variable
...
8. <a href="./intern/intensity.py">intensity.py</a> Calculate the Intensity over heatwaves
...
![Example Run](image_url "example.png")
